...
def CreatePattern(no_of_images, save_folder='objects', pattern='circle', i_size=256, 
                  disk_radius=6, count_begin=35, count_end=50, size=(10,10), 
                  tri_shape=(10,10), f_save=True, var_size=False):
    """
    ...
    for j in range(no_of_images):
        ...
        for i in range(count):
            ...
            if (pattern == 'circle'):
                ...
                point, tri_side, tri_height = CreateTriangle((image_size, image_size), tri_shape,
                                                             rng)
                indexes = polygon([point[0], point[0]-tri_height, point[0],],
                                  [point[1], point[1]+tri_side//2, point[1]+tri_side,]) 
                
            else:
                print(f"The pattern {pattern} mentioned is not implemented.Choose from circle, triangle and square")
                break
...
def CreateMixture(no_of_images, save_folder, add_sq=False, add_tri=False,
                  add_ell=False, i_size=256, disk_radius=6, count_begin=35, count_end=50, 
                  rect_count=10,size=(10,10), tri_count=10, ellip_count=10, major_radius=5, 
                  minor_radius=8, angle=45, tri_shape=(10,10), f_save=True, var_size=False,
                 change_intensity=False, signal_ratio=1, count_percent=1.0):
    """
    ...
    for j in range(no_of_images):
        ...
        if add_ell:
            ...
            for s in range(e_count):
                center = tuple(map(tuple, (np.random.randint(0, center_num, size=(1,2)))))
                rr, cc = ellipse(center[0][0], center[0][1], 5, 8, rotation=np.deg2rad(angle))
                img[rr,cc] = pixel_value
                mask[rr,cc] = 1
...
def EllipsePattern(no_of_images, save_folder, i_size=256, major_radius=5, 
                  minor_radius=8, real_count=30, count_begin=10, count_end=20, angle=45,
                  f_save=True):
    """
...
def CreateTriangle(img_shape, shape, rng):
    ...
    st_point = (row, colm)
    calc_side = min(img_shape[1] - st_point[1], st_point[0], 
                    shape[1]) - shape[0] 
